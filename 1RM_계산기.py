...
if st.session_state.is_auth:
    with st.form(key="comment_form", clear_on_submit=True):
        ...
        if submit_comment and new_comment.strip():
            ...
            conn.update(worksheet="comments", data=pd.concat([comments_df, new_c_row], ignore_index=True))
            st.success("🔥 응원 등록 완료!")
            time.sleep(0.5)
            st.rerun()
...
if not st.session_state.is_auth:
    st.subheader("👤 사용자 인증")
    auth_mode = st.radio("로그인 방식", ["기존 사용자", "신규 등록"], horizontal=True)
    if auth_mode == "기존 사용자":
        ...
        n_name = reg_c1.text_input("새 이름")
        n_gender = reg_c2.radio("성별", ["남성", "여성"], horizontal=True)
...
if st.session_state.is_auth:
    ...
    if not my_data.empty:
        ...
        chart = alt.Chart(chart_df).encode(y=alt.Y('exercise_short:N', sort='-x', title=None), x=alt.X('weight:Q', title="lbs")).mark_bar(color="#29b5e8", cornerRadiusEnd=5)
        st.altair_chart(chart, use_container_width=True)
        
        with st.expander("📋 나의 모든 기록 보기 (상세 테이블)"):
            my_exs = sorted(my_data['exercise'].unique().tolist())
            filt_ex = st.selectbox("종목 필터", ["전체"] + my_exs)
            disp_df = my_data if filt_ex == "전체" else my_data[my_data['exercise'] == filt_ex]
    ...
    save_ex = st.selectbox("업데이트할 종목", exercise_list)
    ...
    if p_max > 0:
        ...
        with st.expander("📊 퍼센트별 중량 (퍼센트 계산기)", expanded=True):
            cols = st.columns(3)
            for i, p in enumerate([50, 60, 70, 80, 90, 100]):
                calc_w = round((p_max * p / 100) / 2.5) * 2.5
                cols[i%3].metric(f"{p}%", f"{calc_w} lb")
    
    new_w = st.number_input("중량 입력 (lbs)", value=p_max, step=5.0)
    new_m = st.text_input("메모", placeholder="컨디션 등 기록")
    if st.button("🏋️ 기록 저장", use_container_width=True) and new_w > 0:
        kst_now = datetime.now() + timedelta(hours=9)
        u_rows = df[df['name'] == st.session_state.user_name]
        f_pw = str(u_rows.iloc[-1]['password']) if not u_rows.empty else st.session_state.get('temp_pw', '0000')
