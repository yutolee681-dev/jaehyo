# 2. 구글 시트 연결 및 설정
# secrets.toml에 주소가 있더라도, 코드에서 명시적으로 연결해주는 것이 가장 확실합니다.
SHEET_URL = "https://docs.google.com/spreadsheets/d/1ekqS81gko96DVkrFsBkg2-bQiF3oAcHkXd02oHJQ1R4"

# 연결 객체 생성 (주소를 여기서 미리 지정)
conn = st.connection("gsheets", type=GSheetsConnection)

def get_full_data():
    try:
        # spreadsheet 매개변수를 직접 전달하여 에러 방지
        raw_df = conn.read(
...
if st.session_state.is_auth:
    ...
    if prev_max > 0:
        ...
        with st.expander("📊 훈련 무게 계산 (50% ~ 100%)", expanded=False):
            ...
            html_list = ["<div style='background-color:#f9f9f9;padding:15px;border-radius:10px;border:1px solid #eee;'>"]
            for p in percents:
